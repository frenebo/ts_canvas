/// <reference path="./diffInterfaces.d.ts"/>

let undoDiff: DiffUndoer;

let applyDiff: DiffApplier;

let createDiff: DiffCreator;

(function() {
  undoDiff = <T extends Diffable>(
    after: T,
  ...
  function undoObjectDiff<T extends DiffableObject>(
    ...
    if (diff.changed !== undefined) for (const changedKey in diff.changed) {
      ...
      before[changedKey] = undoDiff(before[changedKey], attrDiff as any);
    }
  ...
  function applyObjectDiff<T extends DiffableObject>(
    ...
    if (diff.changed !== undefined) for (const changedKey in diff.changed) {
      ...
      after[changedKey] = applyDiff(before[changedKey], attrDiff as any);
    }
...
if (typeof module !== "undefined") {
  module.exports.undoDiff = undoDiff;
  module.exports.applyDiff = applyDiff;
  module.exports.createDiff = createDiff;
}
